...
Public Class frmMain
    ...
    Private Sub accessMenu()
        ...
        ElseIf (gTestMode And gWriteVotesToTest) Then
            ...
            If SVotePC_On = False Then
                BuildVoteIDTS.Visible = True
            Else
                BuildVoteIDTS.Visible = False
            End If
        End If
        ...
        If UCase(gDataComputerName) = UCase(SVOTE) Then
            ...
            Me.SpecialOCalendar.Visible = False
            Me.BuildVoteIDTS.Visible = False
    ...
    Private Sub reportTSroll_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles reportTSroll.Click
        ...
        If v_id = "" Or Val(v_id) = 0 Then
            ...
            Exit Sub
        Else
            If GetLastLocalVoteID() = v_id Then
                RollOfTheSenateByVoteIDReport(v_id)
            Else
                MsgBox("System only prints last voted id: " & v_id & " report.", vbInformation, "Printe Report")
                Me.reportTSvoteID.Text = "Vote ID:"
                Exit Sub
            End If
        End If
    ...
    Private Sub uTSdownloadS_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles uTSdownloadS.Click
        ...
        frmSenatorList.MdiParent = Me
        ChangeSize(frmSenatorList)
        frmSenatorList.Show()
    End Sub
    ...
    Private Sub LoadReport(ByVal rptName As String, Optional ByVal intVoteID As Object = 0)
        ...
        Try
            ...
            End If
        Catch ex As Exception
            RetValue = DisplayMessage(ex.Message, "Print Report", "S")
            Exit Sub
        End Try
    End Sub

    Private Sub LoadVoteIDReport(ByVal rptName As String)
        '--- You can change more print options via PrintOptions property of ReportDocument
        Dim report1 As New ReportDocument()
    ...
    Private Sub RollOfTheSenateByVoteIDReport(ByVal intVoteID As Integer)
        ...
        Dim ds, dsRollCallV As New DataSet
        ...
        If gTestMode Then
            ds = V_DataSet("Select * From tblRollCallVotes WHERE SessionID =" & gSessionID & " AND VoteID =" & intVoteID & " AND TEST=1", "R")
        Else
            ds = V_DataSet("Select * From tblRollCallVotes WHERE SessionID =" & gSessionID & " AND VoteID =" & intVoteID & " AND TEST=0", "R")
        End If
        ...
        If gTestMode Then
            strSQL = "Select * From tblRollCallVotes WHERE TEST = 1 AND VoteID =" & intVoteID & " AND SessionID = " & gSessionID
        Else
            strSQL = "Select * From tblRollCallVotes WHERE TEST = 0 AND VoteID =" & intVoteID & " AND SessionID = " & gSessionID
        End If
        ...
        If gTestMode Then
            dsRC = V_DataSet("Select * From tblRollCallVotes Where TEST =1 AND SessionID =" & gSessionID & " AND VoteID =" & intVoteID, "R")
        Else
            dsRC = V_DataSet("Select * From tblRollCallVotes Where TEST =0 AND SessionID =" & gSessionID & " AND VoteID =" & intVoteID, "R")
        End If
